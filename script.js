...
function loadShader(gl, type, source) { const shader = gl.createShader(type); gl.shaderSource(shader, source); gl.compileShader(shader); if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) { console.error("Error compiling shader: " + gl.getShaderInfoLog(shader)); gl.deleteShader(shader); return null; } return shader; }
function initShaderProgram(gl, vsSource, fsSource) { const vertexShader = loadShader(gl, gl.VERTEX_SHADER, vsSource); const fragmentShader = loadShader(gl, gl.FRAGMENT_SHADER, fsSource); const shaderProgram = gl.createProgram(); gl.attachShader(shaderProgram, vertexShader); gl.attachShader(shaderProgram, fragmentShader); gl.linkProgram(shaderProgram); if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { console.error("Error linking shader program: " + gl.getProgramInfoLog(shaderProgram)); return null; } return shaderProgram; }
function createCheckerboardTexture(gl) { const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d'); const textureSize = 64; const checkSize = textureSize / 2; canvas.width = textureSize; canvas.height = textureSize; const color1 = "#C8C8C8"; const color2 = "#969696"; for (let i = 0; i < 2; i++) { for (let j = 0; j < 2; j++) { ctx.fillStyle = (i + j) % 2 === 0 ? color1 : color2; ctx.fillRect(i * checkSize, j * checkSize, checkSize, checkSize); } } const texture = gl.createTexture(); gl.bindTexture(gl.TEXTURE_2D, texture); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas); gl.generateMipmap(gl.TEXTURE_2D); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.REPEAT); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST_MIPMAP_LINEAR); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST); return texture; }

// ============================ Track Generation ============================
function createTrack(length, seed, minGap, maxGap) { let rng = Math.random; if (seed !== undefined) { let m_w = seed; let m_z = 987654321; rng = function() { m_z = (36969 * (m_z & 65535) + (m_z >> 16)) & 0xffffffff; m_w = (18000 * (m_w & 65535) + (m_w >> 16)) & 0xffffffff; let result = ((m_z << 16) + m_w) & 0xffffffff; result /= 4294967296; return result + 0.5; } } const platforms = []; let currentX = 0; let currentY = 0; const platformHeight = 0.5; const startDepth = 100; platforms.push({ x: 0, y: 0, z: startDepth / 2, width: 10, depth: startDepth, height: platformHeight }); let lastPlatformBackEdge = startDepth; for (let i = 0; i < length; i++) { const gap = rng() * (maxGap - minGap) + minGap; const width = rng() * 8 + 4; const depth = rng() * 15 + 10; const newPlatformCenterZ = lastPlatformBackEdge + gap + (depth / 2); const xShift = (rng() - 0.5) * 10; currentX = Math.max(-15, Math.min(15, currentX + xShift)); if (rng() < 0.2) { const yShift = (rng() - 0.5) * 3; currentY += yShift; } platforms.push({ x: currentX, y: currentY, z: newPlatformCenterZ, width: width, depth: depth, height: platformHeight }); lastPlatformBackEdge = newPlatformCenterZ + (depth / 2); } return platforms; }
...
function init() {
  ...
  locationsTexture = { attribs: { aPosition: gl.getAttribLocation(shaderProgramTexture, "aPosition"), aTexCoord: gl.getAttribLocation(shaderProgramTexture, "aTexCoord") }, uniforms: { uProjection: gl.getUniformLocation(shaderProgramTexture, "uProjection"), uView: gl.getUniformLocation(shaderProgramTexture, "uView"), uModel: gl.getUniformLocation(shaderProgramTexture, "uModel"), uTexture: gl.getUniformLocation(shaderProgramTexture, "uTexture") } };
  locationsColor = { attribs: { aPosition: gl.getAttribLocation(shaderProgramColor, "aPosition"), aColor: gl.getAttribLocation(shaderProgramColor, "aColor") }, uniforms: { uProjection: gl.getUniformLocation(shaderProgramColor, "uProjection"), uView: gl.getUniformLocation(shaderProgramColor, "uView"), uModel: gl.getUniformLocation(shaderProgramColor, "uModel") } };
    ...
    for (const p of trackData) {
        ...
        if (carPos[0] >= p.x - halfWidth && carPos[0] <= p.x + halfWidth && carPos[2] >= p.z - halfDepth && carPos[2] <= p.z + halfDepth) {
            if (carPos[1] <= platformTop + 0.3 && carPos[1] >= platformTop - 0.5) {
                onGround = true; carPos[1] = platformTop + 0.3; 
...
function drawTexturedObject(bufferObj, texture, modelMatrix, projectionMatrix, viewMatrix) { if (!bufferObj || !bufferObj.buffer || bufferObj.vertexCount === 0) return; gl.useProgram(shaderProgramTexture); const stride = 5 * Float32Array.BYTES_PER_ELEMENT; gl.bindBuffer(gl.ARRAY_BUFFER, bufferObj.buffer); gl.vertexAttribPointer(locationsTexture.attribs.aPosition, 3, gl.FLOAT, false, stride, 0); gl.enableVertexAttribArray(locationsTexture.attribs.aPosition); gl.vertexAttribPointer(locationsTexture.attribs.aTexCoord, 2, gl.FLOAT, false, stride, 3 * Float32Array.BYTES_PER_ELEMENT); gl.enableVertexAttribArray(locationsTexture.attribs.aTexCoord); gl.uniformMatrix4fv(locationsTexture.uniforms.uProjection, false, projectionMatrix); gl.uniformMatrix4fv(locationsTexture.uniforms.uView, false, viewMatrix); gl.uniformMatrix4fv(locationsTexture.uniforms.uModel, false, modelMatrix); gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, texture); gl.uniform1i(locationsTexture.uniforms.uTexture, 0); gl.drawArrays(gl.TRIANGLES, 0, bufferObj.vertexCount); }
